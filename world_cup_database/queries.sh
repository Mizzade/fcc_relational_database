...
echo "$($PSQL "SELECT t.name FROM games as g LEFT JOIN teams as t ON g.winner_id = t.team_id WHERE g.year = 2018 AND round = 'Final';")"

echo -e "\nList of teams who played in the 2014 'Eighth-Final' round:"
echo "$($PSQL "WITH g AS (SELECT * FROM games WHERE year = 2014 AND round = 'Eighth-Final') SELECT teams.name FROM (SELECT winner_id as team_id FROM g UNION SELECT opponent_id as team_id FROM g) AS all_teams LEFT JOIN teams USING(team_id) ORDER BY name;")"
